async function loginPage() {
    app.innerHTML = `
        <div style="max-width: 400px; margin: 8rem auto; padding: 0 1rem; text-align: center;">
            <h1 class="serif" style="font-size: 2rem; margin-bottom: 2rem;">Welcome back.</h1>
            <div id="login-form">
                <button type="button" id="google-login-btn" class="btn btn-outline" style="width: 100%; display: flex; align-items: center; justify-content: center; gap: 0.5rem; padding: 0.75rem; border: 1px solid var(--border); border-radius: 4px; cursor: pointer; background: white; color: black;">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                        <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                        <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                        <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
                    </svg>
                    Continue with Google
                </button>
            </div>
            <p style="margin-top: 2rem; font-size: 0.9rem; color: var(--text-muted);">No account? <a href="/register" data-link style="color: var(--accent); font-weight: 700; text-decoration: none;">Create one</a></p>
        </div>
    `;


    document.getElementById('google-login-btn').addEventListener('click', async () => {
        try {
            const data = await apiRequest('/auth/google', { method: 'GET' });
            if (data.url) {
                window.location.href = data.url;
            }
        } catch (err) {
            showToast('Failed to initiate Google Login: ' + err.message);
        }
    });
}
